...
if (navigator.geolocation) {
  navigator.geolocation.getCurrentPosition(
    ...
    function (error) {
      ...
      alert(
        "⚠️ Không thể lấy vị trí chính xác. Vui lòng thử dùng điện thoại hoặc kết nối Wi-Fi để định vị tốt hơn."
      );
    },
    {
      ...
      if (data.length > 0) {
        ...
        data.forEach((san) => {
          ...
          marker.bindPopup(`
            ...
            🏠 ${san.dia_chi}<br>
            📞 ${san.hotline}<br>
            💰 ${san.gia_thue_san || "Không rõ"}<br>
            📌 <strong>Trạng thái: ${
              san.trang_thai || "Chưa cập nhật"
            }</strong><br>
...
document.getElementById("btn-nearest").addEventListener("click", () => {
  ...
  navigator.geolocation.getCurrentPosition((pos) => {
    ...
    fetch(
      `http://localhost:3000/geojson/nearest?lat=${latitude}&lng=${longitude}`
    )
      .then((res) => res.json())
      .then((data) => {
        stadiumList.innerHTML = "";
